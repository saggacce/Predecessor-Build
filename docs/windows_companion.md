...
Una revisión creada desde el informe queda vinculada a su sesión de captura incluso cuando no hubo marcadores. Tras adjuntar la grabación, el bloque **Calibración opcional con el replay completo** permite confirmar que se revisó de principio a fin e indicar cuántas reapariciones propias y avisos de habilidad ocurrieron realmente. El servidor cruza ese total con las señales confirmadas y conserva por separado los falsos positivos, los eventos omitidos y las muestras ambiguas. Una muestra con marcadores pendientes o no verificables se excluye del cálculo de cobertura. La cobertura sólo se estima tras veinte eventos reales acumulados en grabaciones completas revisadas.

Cada sesión de captura tiene como máximo una revisión de calibración y volver a importarla reutiliza la existente. Sólo las sesiones `COMPLETED` con modo `VERIFIED_ALLOWED` pueden contribuir; las sesiones abortadas, bloqueadas o no verificadas quedan excluidas.
