...
app.use(helmet({
  ...
  // Disable upgrade-insecure-requests until HTTPS is configured — it blocks
  // all fetch() calls when the server runs on plain HTTP.
  contentSecurityPolicy: process.env.NODE_ENV === 'production' && process.env.HTTPS_ENABLED === 'true'
    ? undefined   // use helmet default CSP (with upgrade-insecure-requests) when HTTPS is active
    : { directives: { ...helmet.contentSecurityPolicy.getDefaultDirectives(), 'upgrade-insecure-requests': [] } },
}));
