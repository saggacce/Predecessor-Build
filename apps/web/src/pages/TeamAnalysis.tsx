...
import { apiClient, type TeamProfile, ApiErrorResponse } from '../api/client';
...
export default function TeamAnalysis() {
  ...
  async function handleSelectTeam(id: string) {
    try {
      ...
      const message = err instanceof ApiErrorResponse ? err.error.message : 'Failed to load team profile.';
      toast.error(message);
    }
  }
                ...
                {group.map((team) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-sm)', cursor: 'pointer', color: 'var(--text-primary)', textAlign: 'left',
                    }}
                  >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="glass-card">
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <span style={{
                  marginLeft: 'auto', fontSize: '0.75rem', fontWeight: 700, padding: '0.25rem 0.75rem', borderRadius: '999px',
              ...
              {selected.roster.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {selected.roster.map((member) => (
                    <div key={member.playerId} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', background: 'var(--bg-dark)', borderRadius: 'var(--radius-sm)' }}>
                      <div>
                      ...
                      {member.rating?.rankLabel && (
                        <span style={{ fontSize: '0.75rem', color: 'var(--accent-purple)', fontWeight: 600 }}>{member.rating.rankLabel}</span>
                      )}
                    </div>
                  ))}
                </div>
